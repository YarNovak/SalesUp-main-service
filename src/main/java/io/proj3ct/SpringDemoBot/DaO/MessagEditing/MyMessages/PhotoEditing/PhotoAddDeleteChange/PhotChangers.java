package io.proj3ct.SpringDemoBot.DaO.MessagEditing.MyMessages.PhotoEditing.PhotoAddDeleteChange;

import io.proj3ct.SpringDemoBot.DB_entities.Bot;
import io.proj3ct.SpringDemoBot.DB_entities.BotMessage;
import io.proj3ct.SpringDemoBot.DaO.CallbackHandler;
import io.proj3ct.SpringDemoBot.DaO.MessagEditing.MyMessages.PhotoEditing.Photo_handler;
...
@Component
public class PhotChangers implements CallbackHandler {
    ...
    public boolean support(String callbackData) {
        return callbackData.startsWith("PHOTO_");
    }

    @Autowired
    private Photo_handler photoHandler;
...
*/
            ...
            photoHandler.add(query.getFrom().getId(), botId, key);
        ...
        else if(parts[0].equals("PHOTO_DELETE")){
      ...
      //      sendMessage.setMessageId(query.getMessage().getMessageId());
         ...
         //   newPhoto.setCaption("");
            ...
            newPhoto.setCaption(escapeMarkdown("\uD83D\uDCF8") +"*_Редактирование фото_*\n" +
                    "\n" +
                    escapeMarkdown("⚙️\uFE0F")+" модифицируйте в любой момент");

            // if() newPhoto.setMedia();
            //else if()
        ...
        else if(parts[0].equals("PHOTO_CHANGE")){
/*
            SendPhoto sendPhoto = new SendPhoto();
            sendPhoto.setChatId(String.valueOf(query.getMessage().getChatId()));
            ...
            ByteArrayInputStream bais = new ByteArrayInputStream(bm.getPhoto());
 ...
 */
            ...
            photoHandler.expectPhotoRename(query.getFrom().getId(), botId, key);
    ...
    private List<List<InlineKeyboardButton>> create_buttons(String key, Long botId, Long type){
        ...
        if(type == 0){
            buttons =  new java.util.ArrayList<>( List.of(
                    List.of(newButton("\uD83D\uDCF8 Добавить", "PHOTO_ADD:" +key  + ":" + botId))
            ));

        }
        else{
            buttons  = new java.util.ArrayList<>( List.of(
                    List.of(newButton("❌УДАЛИТЬ", "PHOTO_DELETE:" +key  + ":" + botId)),
                    List.of(newButton("\uD83D\uDCF8 ИЗМЕНИТЬ", "PHOTO_CHANGE:" +key  + ":" + botId))
            ));
        }
